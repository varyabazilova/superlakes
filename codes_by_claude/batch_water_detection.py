...
import re
from water_detection import read_planet_image, compute_ndwi, create_glacier_mask, otsu_threshold_water, refine_water_mask
import argparse
...
def process_single_image(image_path, glacier_shp, output_dir=None):
    """
    ...
    try:
        ...
        # Apply Otsu thresholding with glacier mask
        water_mask, threshold_val, otsu_analysis = otsu_threshold_water(ndwi, glacier_mask)
        
        # Refine mask
        refined_mask = refine_water_mask(water_mask, min_size=10)
        ...
        if output_dir:
            ...
            # Save water mask
            water_output = os.path.join(output_dir, f"{base_name}_water_mask.tif")
...
def batch_process_images(input_pattern, glacier_shp=None, output_dir=None, output_csv=None):
    """
    Batch process multiple Planet images
    ...
    # Process all images (each creates its own glacier mask)
    results = []
    for i, image_path in enumerate(image_files, 1):
        print(f"\n[{i}/{len(image_files)}] Processing images...")
        stats = process_single_image(image_path, glacier_shp, output_dir)
    ...
    errors = df[df.get('error', pd.Series()).notna()]
    if len(errors) > 0:
...
def main():
    parser = argparse.ArgumentParser(description='Batch water detection on Planet imagery')
    ...
    parser.add_argument('--output_dir', help='Directory to save individual water masks and NDWI files')
    parser.add_argument('--output_csv', default='water_detection_summary.csv', 
                       help='Path to save summary CSV (default: water_detection_summary.csv)')
